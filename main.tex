...
\spirou{revise this section}

It is straightforward to show that of the 21 possible spectra only 9 are in fact nonzero, and that they do form three pairs. Specifically, the cross-correlations $\bra\kappa\gamma_+^\prime\ket = 0$, $\bra\kappa\gamma_\times^\prime\ket = 0$ and $\bra\gamma_+\gamma_\times^\prime\ket = 0$ are zero as well as $\bra s\epsilon_+^\prime\ket = 0$, $\bra s\epsilon_\times^\prime\ket = 0$ and $\bra\epsilon_+\epsilon_\times^\prime\ket = 0$; likewise one gets vanishing cross-correlations $\bra s\gamma_+^\prime\ket = 0$, $\bra s\gamma_\times^\prime\ket = 0$, $\bra\kappa\epsilon_+^\prime\ket = 0$ and $\bra\kappa\epsilon_\times^\prime\ket = 0$ as well as $\bra\gamma_+\epsilon_\times^\prime\ket = 0$ and $\bra\gamma_\times\epsilon_+^\prime\ket = 0$. As consistency relations between the non-vanishing correlations one obtains $\bra\kappa\kappa^\prime\ket = \bra\gamma_+\gamma_+^\prime\ket + \bra\gamma_\times\gamma_\times^\prime\ket$ as well as $\bra ss^\prime\ket = \bra\epsilon_+\epsilon_+^\prime\ket + \bra\epsilon_\times\epsilon_\times^\prime\ket$. Fig.~\ref{fig:sizesize} shows the intrinsic and extrinsic size-spectra, as they would result from a tomographic survey. Similarly to the shape-spectra discussed in the previous section, we choose a negative value for the alignment parameter $D$ in order to not having to carry through minus-signs explicitly.
...
% --- section:  --- %
\section{cross correlations between shapes and sizes}\label{sect_cross}


% --- section:  --- %
\section{information content of shape and size correlations}\label{sect_fisher}
For quantifying the information content of intrinsic size and shape correlations in comparison to weak lensing convergence and shear we use the Fisher-matrix formalism. Arranging the measurements of galaxy shapes and sizes into a data vector yields a data covariance matrix which is diagonal due to the vanishing cross-correlations between size and shape,
\begin{equation}
C =
\left(
\begin{array}{cc}
C_\mathrm{shape}(\ell) & 0 \\
0 & C_\mathrm{size}(\ell)
\end{array}
\right)
\end{equation}
