...
\begin{equation}
\phi_{A,ab} = \partial_a\partial_b\phi_A
\quad\mathrm{with}\quad
\phi_A = \int\dd\chi\:p_A(\chi)\chi^2\frac{c}{H(\chi)}\:\Phi = \int\dd\chi\:W_{\phi,A}(\chi)\:\Phi,
\end{equation}
with the Hubble-function $H(\chi)$. From that, one can recover the ellipticity components $\epsilon_{+,A}$ and $\epsilon_{\times,A}$ as well as the size $s_A$ from a decomposition of the tensor $\phi_{A,ab}$ with the Pauli-matrices $\sigma_{ab}^{(n)}$,
\begin{equation}
\phi_{A,ab} = s_A\sigma^{0}_{ab} + \epsilon_{+,A}\sigma^{(1)}_{ab} + \epsilon_{\times,A}\sigma^{(3)}_{ab},
\mathrm{~with}\quad
\sigma^{(0)} = \left(
\begin{array}{cc}
1 & 0 \\ 0 & 1
...
\right)
\end{equation}
where three components are sufficient because of the symmetry $\phi_{A,ab} = \phi_{A,ba}$. Using two properties of the Pauli-matrices $\sigma_{ab}^{(n)}$, namely $\sigma_{ab}^{(m)}\sigma_{bc}^{(n)} = \delta_{mn}\sigma^{(0)}_{ac} + \epsilon_{mno}\sigma^{(o)}_{ac}$, and their tracelessness $\sigma^{(m)}_{aa} = 0$, it is possible to invert the last relation and to obtain the expansion coefficients,
\begin{equation}
s_A = \frac{1}{2}\phi_{A,ab}\sigma^{(0)}_{ab},
\quad
\epsilon_{+,A} = \frac{1}{2}\phi_{A,ab}\sigma^{(1)}_{ab},
\mathrm{~and}\quad
\epsilon_{\times,A} = \frac{1}{2}\phi_{A,ab}\sigma^{(3)}_{ab}.
\end{equation}
...
This implies that the statistics of all modes of the shape and size field can be described by spectra of the source fields, which in turn are given by a Limber-projection. Specifically, the spectrum of $\phi_{A,ab}$ reads
\begin{equation}
\bra\phi_{A,ab}(\bmath\ell)\phi_{B,ij}^*(\bmath\ell^\prime)\ket = 
(2\pi)^2\dirac(\bmath\ell-\bmath\ell^\prime)\:C^{\phi_A\phi_B}_{abij}(\ell)
\quad\mathrm{with}\quad
C^{\phi_A\phi_B}_{abij}(\ell) = 
\ell_a\ell_b\ell_i\ell_j\:\int\frac{\dd\chi}{\chi^2}\:W_{\phi,A}(\chi)W_{\phi,B}(\chi)\:P_{\Phi\Phi}(k = \ell/\chi),
\end{equation}
...
\begin{equation}
\bra\phi_{A,ab}(\bmath\ell)\psi_{B,ij}^*(\bmath\ell^\prime)\ket =
(2\pi)^2\dirac(\bmath\ell-\bmath\ell^\prime)\:C^{\phi_A\psi_B}_{abij}(\ell)
\quad\mathrm{with}\quad
C^{\phi_A\psi_B}_{abij}(\ell) =
\ell_a\ell_b\ell_i\ell_j\:\int\frac{\dd\chi}{\chi^2}\:W_{\phi,A}(\chi)W_{\psi,B}(\chi)\:P_{\Phi\Phi}(k = \ell/\chi).
\end{equation}
...
for intrinsic alignments,
\begin{equation}
C^{s\kappa}_{AB}(\ell) = \frac{1}{4}\sigma^{(0)}_{ab}\sigma^{(0)}_{ij}C^{\phi_A\psi_B}_{abij}(\ell)
\end{equation}
and again, for the cross-correlation between the two,
\begin{equation}
C^{ss}_{AB}(\ell) = \frac{1}{4}\sigma^{(0)}_{ab}\sigma^{(0)}_{ij}C^{\phi_A\phi_B}_{abij}(\ell)
\end{equation}
