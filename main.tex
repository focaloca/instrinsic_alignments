...
\end{equation}
with the noiseless spectrum $S(\ell)$ of which the signal strength is sought. For the case of Euclid, we extend the summation over the multipoles from $\ell=10$ to $\ell=3000$, and we are assuming a full-sky coverage for simplicity with no correlations between different multipoles, but scale down the signal subsequently with a sky coverage of $f_\mathrm{sky}=0.3$, which would be justified because most of the signal originates at small angular scales.
